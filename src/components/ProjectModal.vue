<template>
    <transition name="dissolve" appear>
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div @click.stop class="relative  w-[80vw] mx-4 bg-white rounded-lg shadow-xl overflow-hidden p-0">
                <button @click="close" aria-label="Close modal"
                    class="absolute top-3 right-3 z-30 text-gray-600 hover:text-gray-900 p-1">
                    <i class="fa-solid fa-xmark text-2xl cursor-pointer" aria-hidden="true"></i>
                </button>

                <div class="max-h-[80vh] bg-white overflow-hidden">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 p-8 items-start">
                        ...
                        <!-- Middle: main content -->
                        <main class="md:col-span-7 max-h-[70vh] overflow-auto p-4">
                            <h2 class="text-2xl font-extrabold text-gray-900 mb-4">
                                <Underline>{{ project?.title }}</Underline>
                            </h2>
                            <div class="divider border-t border-gray-200 mb-4"></div>
                            <p class="text-gray-600 text-sm">{{ project?.description }}</p>
                            <p class="text-gray-400 text-xs mt-2">{{ project?.credits }}</p>
                        ...
                        <aside class="md:col-span-2 self-start h-[70vh]">
                            <div class="flex flex-col gap-3">
                                <div class="p-4 bg-[#fafafa] rounded mt-8">

                                    <Underline custom-class="bottom-10">
                                        <h4 class="font-semibold text-sm text-gray-700">Tools used</h4>
                                    </Underline>
